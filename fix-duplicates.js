...
if (fs.existsSync(toolsDir)) {
  ...
  items.forEach(item => {
    ...
    if (fs.existsSync(pagePath)) {
      ...
      // 1. Identify and remove all duplicate/incorrectly paired Enterprise sections
      content = content.replace(/\{?\/\*[\s\S]*?(?:Enterprise-Grade|Visible Professional)[\s\S]*?\*\/\}?\s*<div[\s\S]*?<\/div>\s*<\/div>/g, '');
      
      // 2. Extract tool name nicely from folder name for a single, correct heading
      const niceName = item.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
      
      // 3. Generate the single, correct, unified Enterprise-Grade section
      const singleEnterpriseSection = `
    {/* Unified Enterprise-Grade Standards & Features Section */}
    <div className="w-full max-w-7xl mx-auto px-4 py-12 mt-12 border-t border-slate-800">
      ...
      // 4. Inject the single, correct section at the end before component closure
      const lastDivIndex = content.lastIndexOf('</div>');
      if (lastDivIndex !== -1) {
        content = content.substring(0, lastDivIndex) + singleEnterpriseSection + '\n    </div>' + content.substring(lastDivIndex + 6);
        fs.writeFileSync(pagePath, content, 'utf8');
      }
    }
  });
  console.log('Duplicates removed, single correct Enterprise section and feedback block injected into all tools successfully!');
}
