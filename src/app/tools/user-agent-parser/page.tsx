...
export default function UserAgentParserPage() {
  ...
  const [device, setDevice] = useState('Desktop / Unknown');
  ...
  const parseUa = (ua: string) => {
    // Basic Client-side parsing heuristics
        ...
        'mainEntity': [
          {
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Paste any User-Agent string or use your current browser signature to instantly analyze browser, OS, and device type.'
            }
          },
          {
            '@type': 'Question',
            ...
            'acceptedAnswer': {
              '@type': 'Answer',
              'text': 'Yes! Everything is processed locally on your client machine with absolute privacy.'
            }
          }
        ]
      }
          ...
          <div style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '24px', marginTop: 'var(--space-6)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
              <div>
                ...
                <textarea value={uaString || ''} onChange={(e) => { setUaString(e.target.value); try { handleManualParse(e.target.value); } catch(err) {} }} rows={10} placeholder="Enter your input here..." style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', padding: '12px', fontFamily: 'monospace', fontSize: '0.85rem', outline: 'none' }} />
              </div>
              <div>
...
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <textarea value={browser || ''} readOnly rows={10} placeholder="Output will appear here..." style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#34d399', padding: '12px', fontFamily: 'monospace', fontSize: '0.85rem', outline: 'none' }} />
              </div>
            </div>
          </div>
