...
export default function Page() {
  const [input, setInput] = useState("https://example.com/path/to/page?search=hello&page=2#section");
  ...
  const handleProcess = (val: string) => {
    ...
    try {
      const fn = function urlParser(val) {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "name": "VelnoxLabs URL Parser & Query Extractor",
        "operatingSystem": "All",
        "applicationCategory": "DeveloperApplication",
        "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" },
        "description": "Deconstruct URLs into components and query params."
      }) }} />

      <GlobalContainer maxWidth="2xl">
        <div style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--space-16)' }}>
          <SectionHeading title="URL Parser & Query Extractor" subtitle="Deconstruct URLs into components and query params." />
          <div style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '24px', marginTop: 'var(--space-6)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
              <div>
                <label style={{ color: '#fff', fontSize: '0.875rem', fontWeight: 600, display: 'block', marginBottom: '8px' }}>URL:</label>
                <textarea value={input} onChange={(e) => handleProcess(e.target.value)} rows={12} placeholder="Paste a URL..." style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', padding: '12px', fontFamily: 'monospace', fontSize: '0.85rem', outline: 'none' }} />
                {error && <p style={{ color: '#f87171', fontSize: '0.8rem', marginTop: '8px' }}>{error}</p>}
              </div>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <label style={{ color: '#fff', fontSize: '0.875rem', fontWeight: 600 }}>Parsed Components:</label>
          ...
          <div style={{ marginTop: '48px', backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '32px' }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#ffffff', marginBottom: '16px' }}>What is a URL Parser & Query Extractor?</h2>
            <p style={{ color: '#94a3b8', lineHeight: 1.7, marginBottom: '24px' }}>
              The URL Parser deconstructs URLs into protocol, host, path, and query parameters. It helps developers debug routing and inspect tracking parameters.
            </p>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <p style={{ color: '#94a3b8', lineHeight: 1.6 }}>Yes, VelnoxLabs URL Parser & Query Extractor is 100% free with no sign-up required.</p>
            </div>
