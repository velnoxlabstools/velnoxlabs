...
export default function Page() {
  const [input, setInput] = useState("default-src='self'\nscript-src='self' https://cdn.example.com\nstyle-src='self' 'unsafe-inline'");
  ...
  const handleProcess = (val: string) => {
    ...
    try {
      const fn = function cspBuilder(val) {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "name": "VelnoxLabs CSP Header Builder",
        "operatingSystem": "All",
        "applicationCategory": "DeveloperApplication",
        "offers": { "@type": "Offer", "price": "0", "priceCurrency": "USD" },
        "description": "Build and validate Content Security Policy headers securely."
      }) }} />

      <GlobalContainer maxWidth="2xl">
        <div style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--space-16)' }}>
          <SectionHeading title="CSP Header Builder & Validator" subtitle="Build Content Security Policy headers." />
          <div style={{ backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '24px', marginTop: 'var(--space-6)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
              <div>
                <label style={{ color: '#fff', fontSize: '0.875rem', fontWeight: 600, display: 'block', marginBottom: '8px' }}>Directives (one per line):</label>
                <textarea value={input} onChange={(e) => handleProcess(e.target.value)} rows={12} placeholder="default-src='self'" style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', padding: '12px', fontFamily: 'monospace', fontSize: '0.85rem', outline: 'none' }} />
          ...
          <div style={{ marginTop: '48px', backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '32px' }}>
            ...
            <p style={{ color: '#94a3b8', lineHeight: 1.7, marginBottom: '24px' }}>
              The CSP Header Builder & Validator helps developers construct Content Security Policy headers to prevent XSS attacks, clickjacking, and code injection. It validates your directives and outputs a production-ready header string.
            </p>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <p style={{ color: '#94a3b8', lineHeight: 1.6 }}>Yes, VelnoxLabs CSP Header Builder is 100% free with no sign-up required.</p>
            </div>
